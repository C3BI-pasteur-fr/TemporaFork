...
cortex_tempora <- CalculatePWProfiles(cortex_tempora,
                                      gmt_path = "Mouse_GOBP_AllPathways_no_GO_iea_March_01_2021_symbol.gmt",
...
data <- getExpr(scInput, "logcounts")
data <- as.matrix(data)
metadata <- getMD(scInput)
colnames(metadata)[which(colnames(metadata) == "label")] <- "Clusters"
colnames(metadata)[which(colnames(metadata) == "timePoints")] <- "Timepoints"
tempora_obj <- CreateTemporaObject(data, meta.data = metadata,
                                   timepoint_order = timepoint_order, cluster_labels = cluster_labels,
                                   cell_markers = cell_markers, threshold = threshold)
exprMatrix = data
meta.data = metadata
...
clustmd <- reshape2::dcast(as.data.frame(clustmd), Clusters ~ Timepoints, value.var = "Clusters",
                           fun.aggregate = length)

clustmd[, 2:ncol(clustmd)] <- t(apply(clustmd[, 2:ncol(clustmd)],
                                      1, function(x) x/sum(x)))
clustmd$Cluster_time_score <- apply(clustmd[, 2:ncol(clustmd)],
                                    1, function(x) sum(mapply(function(t, y) t * y, as.numeric(x),
                                                              sort(unique(meta.data$Timescore), decreasing = F))))
colnames(clustmd)[1] <- "Id"
if (!is.null(cluster_labels) & !is.null(cell_markers)) {
  clustmd$label <- paste0("Cluster ", paste(rownames(clustmd),
                                            cluster_labels, sep = "-"))
  ...
  cluster_labels <- IdentifyCellTypes(exprMatrix, cluster_labels = cluster_number,
                                      cell_markers = cell_markers, threshold = threshold)
  clustmd$label <- paste0("Cluster ", paste(rownames(clustmd),
                                            cluster_labels, sep = "-"))
}else {
...
tempora <- new("Tempora", data = exprMatrix, meta.data = meta.data,
